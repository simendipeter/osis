...
class TestLoadTeachingMaterial(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.achievements = [TeachingMaterialFactory(title="Title {}".format(idx),
                                                    learning_unit_year=cls.l_unit_1) for idx in range(5)]

    def test_load_teaching_material(self):
    ...
    def test_load_achievements_check_order(self):
        results = load_teaching_materials(self.l_unit_1.acronym, self.l_unit_1.academic_year.year)
        for idx in range(5):
            self.assertEqual(results[idx].title, "Title {}".format(idx))
