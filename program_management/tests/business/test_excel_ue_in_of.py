...
class TestGenerateEducationGroupYearLearningUnitsContainedWorkbook(TestCase):
    ...
    def setUpTestData(cls):
        ...
        #     EducationGroupYearLearningUnitsContainedToExcel(CustomXlsForm({}),
        #                                                     cls.root.academic_year.year,
        ...
        #TODO peut-être construire l'arbre via les factory?

        # cls.hierarchy = load_tree.load_from_year_and_code(cls.education_group_yr_root.academic_year.year,
        #                                                   cls.education_group_yr_root.partial_acronym)
...
class TestContent(TestCase):
    ...
    def setUpTestData(cls):
        ...
        #TODO je n'arrive pas à utiliser la factory???
        cls.ue_entities = Entities(requirement_entity_acronym='ILV', allocation_entity_acronym='DRT')

        cls.teacher_1 = Teacher(last_name='Dupont', first_name="Marcel", email="dm@gmail.com")
        cls.attribution_1 = Attribution(teacher=cls.teacher_1)
        cls.teacher_2 = Teacher(last_name='Marseillais', first_name="Pol", email="pm@gmail.com")

        cls.attribution_2 = Attribution(teacher=cls.teacher_2)
        DddLearningUnitYearFactory
        # cls.luy = DddLearningUnitYear(
    ...
    def test_fix_data(self):
        ...
        res = _fix_data(self.link_1_1, self.luy, {DIRECT_GATHERING_KEY: self.child_node, MAIN_GATHERING_KEY: self.parent_node, EXCLUDE_UE_KEY: False})
        self.assertEqual(res, expected)

    def test_no_main_parent_result(self):
        link = LinkFactory()
        root_node = link.parent
        subgroup_node = link.child
        tree = ProgramTreeFactory(root_node=root_node)
    ...
    def test_main_parent_not_direct(self):
        ...
        self.assertEqual(get_explore_parents([group_level_1, root_node])[MAIN_GATHERING_KEY], group_level_1)

        group_level_2 = NodeGroupYearFactory(category=Categories.GROUP)
        link = LinkFactory(parent=group_level_1,
                           child=group_level_2)

        ue_level_3 = NodeLearningUnitYearFactory()
        link = LinkFactory(parent=group_level_2,
                           child=ue_level_3)
        tree = ProgramTreeFactory(root_node=root_node)
        self.assertEqual(get_explore_parents([group_level_2, group_level_1, root_node])[MAIN_GATHERING_KEY], group_level_1)
    ...
    #     #TODO je vois pas comment valider l'ordre
    ...
    # return "{} - {}".format(direct_gathering_node.code, direct_gathering_node.group_title_fr or '') if direct_gathering_node else ''
...
class TestExcludeUEFromdWorkbook(TestCase):
    ...
    #     exl = EducationGroupYearLearningUnitsContainedToExcel(CustomXlsForm({}),edy, edy)
...
def get_expected_data_new(child_node, luy, link, main_gathering=None):
    ...
    if main_gathering:
        main_gathering_str = "{} - {}".format(main_gathering.title,
                                    main_gathering.offer_partial_title_fr or '' if main_gathering.is_finality() else main_gathering.group_title_fr) if main_gathering else ''
    else:
        main_gathering_str = ''
    #
    # def _build_gathering_content(edg: 'NodeGroupYear') -> str:
    #     return "{} - {}".format(edg.code, edg.group_title_fr or '') if edg else ''
    #
    # def _build_main_gathering_content(edg: 'Node') -> str:
    #     if edg:
    #         return "{} - {}".format(edg.title,
    #                                 edg.offer_partial_title_fr or '' if edg.is_finality() else edg.group_title_fr) if edg else ''
    #     else:
    #         return ''
...
class TestRowHeight(TestCase):
    ...
    def setUpTestData(cls):
        ...
        second_ue_level_group_level_1_1 = NodeLearningUnitYearFactory(node_id=5, code='ue2', year=cls.academic_year.year)
        link = LinkFactory(parent=cls.group_level_1_1,
                           child=second_ue_level_group_level_1_1)

        cls.group_level_2 = NodeGroupYearFactory(node_id=6, node_type=TrainingType.MASTER_MS_120, year=cls.academic_year.year)
        link = LinkFactory(parent=cls.root_node,
                           child=cls.group_level_2)

        cls.group_level_2_1 = NodeGroupYearFactory(node_id=7, node_type=GroupType.COMMON_CORE, year=cls.academic_year.year)
        ...
        cls.group_level_2_2 = NodeGroupYearFactory(node_id=8, node_type=GroupType.OPTION_LIST_CHOICE, year=cls.academic_year.year)
        link = LinkFactory(parent=cls.group_level_2,
        ...
        element_ue_1 = ElementLearningUnitYearFactory(id=cls.ue_level_group_level_1_1.node_id,
                                                      learning_unit_year=LearningUnitYearFactory(acronym='ue1', academic_year=cls.academic_year))
        element_ue_2 = ElementLearningUnitYearFactory(id=second_ue_level_group_level_1_1.node_id,
                                                      learning_unit_year=LearningUnitYearFactory(acronym='ue2', academic_year=cls.academic_year))
        element_ue_3 = ElementLearningUnitYearFactory(id=cls.ue_level_group_level_2_1.node_id,
                                                      learning_unit_year=LearningUnitYearFactory(acronym='ue3', academic_year=cls.academic_year))
        element_ue_4 = ElementLearningUnitYearFactory(id=cls.ue_level_group_level_2_2.node_id,
    ...
    def test_do_not_exclude_options_list_if_not_2M(self):
        ...
        link = LinkFactory(parent=bachelor_root_node,
                           child=group_level_1)
        ...
        link = LinkFactory(parent=group_level_1_1,
                           child=ue_level_group_level_1_1)
        second_ue_level_group_level_1_1 = NodeLearningUnitYearFactory(code='ue22', node_id=101, year=self.academic_year.year)
        link = LinkFactory(parent=group_level_1_1,
                           child=second_ue_level_group_level_1_1)

        group_level_2 = NodeGroupYearFactory(node_id=6, node_type=GroupType.OPTION_LIST_CHOICE, year=self.academic_year.year)
        link = LinkFactory(parent=bachelor_root_node,
                           child=group_level_2)
        ...
        ElementLearningUnitYearFactory(id=ue_level_group_level_1_1.node_id,
                                       learning_unit_year=LearningUnitYearFactory(acronym='ue21', academic_year=self.academic_year))
        ElementLearningUnitYearFactory(id=second_ue_level_group_level_1_1.node_id, learning_unit_year=LearningUnitYearFactory(acronym='ue22', academic_year=self.academic_year))
        ElementLearningUnitYearFactory(id=ue_level_group_level_2.node_id, learning_unit_year=LearningUnitYearFactory(acronym='ue23', academic_year=self.academic_year))
