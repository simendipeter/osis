...
class EducationGroupYearLearningUnitsContainedToExcel:
    ...
    def __init__(self, custom_xls_form: CustomXlsForm, year: int, code: str):
        # todo voir si je peux supprimer les if quand l'arbre fonctionnera
...
def _build_excel_lines_ues(custom_xls_form: CustomXlsForm, tree: 'ProgramTree'):
    ...
    for path, child_node in tree.root_node.descendents.items():
        if isinstance(child_node, NodeLearningUnitYear):
            ...
            print(child_node.year)
            learning_unit_years = load_multiple_by_identity([LearningUnitYearIdentity(code=child_node.code, year=child_node.year)])
            print("lg {}".format(len(learning_unit_years)))
            if learning_unit_years:
                ...
                if not parents_data[EXCLUDE_UE_KEY]:
                    content.append(_get_optional_data(_fix_data(link, luy, parents_data), luy, optional_data_needed, link))
...
def _build_main_gathering_label(main_gathering_node: 'Node') -> str:
    return "{} - {}".format(
        main_gathering_node.title,
        main_gathering_node.offer_partial_title_fr if main_gathering_node.is_finality() else main_gathering_node.group_title_fr) if main_gathering_node else ''
...
def get_explore_parents(parents_of_ue: List['Node']) -> Dict[str, 'Node']:
    ...
    if parents_of_ue:
        ...
        for parent in parents_of_ue:
            ...
            if parent.category and parent.category.name in [education_group_categories.TRAINING,
